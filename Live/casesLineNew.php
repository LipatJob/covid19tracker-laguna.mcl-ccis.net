<?php
  ...
  $result = mysqli_query($con,"SELECT MAX(ref_date) as reference_date FROM reference_dates;");
  ...
  $string = "SELECT refDates.ref_date as reference_date, sum(current_positive_case) AS current,sum(total_positive_cases) AS TOTAL_POSITIVE_CASES, sum(current_deceased) AS TOTAL_DECEASED, sum(current_recovered) AS TOTAL_RECOVERED FROM barangay_history_new as brgynew
    ...
    WHERE refDates.ref_date >= '2020-03-20' ";
  // $string2 = $string;
  ...
  if($location != "LAGUNA")
  {
    ...
    $string.="AND city.CityName = '$location' GROUP by refDates.ref_date";
