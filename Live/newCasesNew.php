...
$pum = array();
  ...
  if($location != "LAGUNA")
  {
    ...
    $string .= "SELECT refDates.ref_date AS reference_date, sum(casesO.current_PUI) AS PUI
                    FROM barangay_history_new as brgynew
                        INNER JOIN Old_Cases as casesO
                        ...
                        WHERE city.CityName = '" . $location . "' and refDates.ref_date >= '2020-03-20'
                        GROUP BY refDates.ref_date
        UNION ALL
        
        SELECT refDates.ref_date AS reference_date, sum(casesN.total_PUI) AS PUI
                    FROM barangay_history_new as brgynew
                        ...
                        on brgynew.refDateID = refDates.ID 
                        WHERE city.CityName = '" . $location . "' and refDates.ref_date >= '2020-03-20'
                        GROUP BY refDates.ref_date ORDER BY reference_date ASC ";
  }
  else
  {
