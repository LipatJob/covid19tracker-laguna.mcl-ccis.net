...
if(isset($_POST["func"]) && !empty(isset($_POST['func']))){
    ...
    switch ($functionName) {
        ...
        case 'updateSpecificDate':
            callUpdateSpecificDate($data);
            break;
        case 'updateIndividualCases':
            callUpdateIndividualCases($data);
            break;
        case 'updateBarangayHistory':
            callUpdateBarangayHistory($data);
            break;
        case 'updateBarangayHistoryNew':
            callUpdateBarangayHistoryNew($data);
            break;
        case 'clearCache':
            callUpdateSpecificDate($data);
            break;
        case 'setUploadingInterface':
            callUpdateSpecificDate($data);
            break;
    }
...
echo "<br><a href = 'index.php'>Return to uploader</a>";
...
/**
* Calls updateBarangayHistory with the API links
*/
...
/**
* Calls updateBarangayHistoryNew with the API links
*/
...
/**
* Calls updateToday with the API links
*/
function callUpdateToday(){
    if(!file_exists("links.json")){
        echo "links.json is not found please contact the developers";
        return;
    }
    $links =  json_decode(file_get_contents("links.json"), true);
    $overviewLinks = $links["overview"];
    updateToday($overviewLinks);
}

/**
* Calls update updateSpecificDate with the API links
*/
function callUpdateSpecificDate($date){
    if(!file_exists("links.json")){
        echo "links.json is not found please contact the developers";
        return;
    }
    $links =  json_decode(file_get_contents("links.json"), true);
    $overviewLinks = $links["overview"];
    updateSpecificDate($overviewLinks, $date);
}
...
function updateToday($apiLinks){
    return updateSpecificDate($apiLinks, date("Y-m-d"));
}

function updateSpecificDate($apiLinks, $dateToUpdate){
    $con = getConnection();       
    mysqli_autocommit($con, FALSE); 
    
    //Start Transaction
    $startTrans = 'START TRANSACTION;';
    mysqli_query($con, $startTrans); 
    // Refdate ID
    $refDateQuery =  "SELECT ID FROM `reference_dates` WHERE ref_date = '{$dateToUpdate}'";
    $refDateRes = mysqli_query($con, $refDateQuery);
    
    // Delete Refdate
    if(mysqli_num_rows($refDateRes) > 0){
        $refDate =  mysqli_fetch_array($refDateRes)["ID"];
        $deleteRefDateQuery = "DELETE FROM `barangay_history_new` where `refDateID` = {$refDate}";
        mysqli_query($con, $deleteRefDateQuery);
        if(mysqli_error($con)){
    ...
    foreach ($apiLinks as $link) {
        ...
        $apiContent = file_get_contents($link);
        $lines = explode(PHP_EOL, $apiContent);
        $START_LINE = floor(count($lines)/2);
        for ($currentLine = $START_LINE; $currentLine < count($lines); $currentLine++) {
            $line = $lines[$currentLine];
            $current = str_getcsv($line);
            
            //extract data from line of csv
            $referenceDate = getSqlStr($current[0]);
            $refDate = DateTime::createFromFormat('Y-m-d', $current[0]);
            $toUpdate = DateTime::createFromFormat('Y-m-d', $dateToUpdate);
            $dateDiff = $refDate->diff($toUpdate)->format('%a');
            # Update only the specified date
            if($dateDiff != 0){
                continue;
            }
